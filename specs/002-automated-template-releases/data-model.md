...
This document defines the key entities, their relationships, and validation rules for the automated template release generation system. These entities represent the domain model for generating, validating, and publishing twitter-Kit templates across 18 AI agents and 2 script variants.
...
- `command_files` MUST all start with "twitterkit." prefix
...
└── commands/ (or prompts/, workflows/)
    ├── twitterkit.specify.md (REQUIRED)
    ├── twitterkit.plan.md (REQUIRED)
    ├── twitterkit.tasks.md (REQUIRED)
    ├── twitterkit.implement.md (REQUIRED)
    ├── twitterkit.clarify.md (REQUIRED)
    ├── twitterkit.analyze.md (REQUIRED)
    ├── twitterkit.checklist.md (REQUIRED)
    ├── twitterkit.taskstoissues.md (REQUIRED)
    └── twitterkit.constitution.md (REQUIRED)
```
...
- `file_name` (string, required): Full filename (e.g., "twitterkit.specify.md")
- `agent` (string, required): Agent namespace (MUST be "twitterkit.{command_name}")
...
**Validation Rules**:
- `file_name` MUST match pattern `twitterkit.{command_name}.{extension}`
- `agent` MUST match pattern `twitterkit.{command_name}` (NOT `speckit.*`)
- `description` MUST be 3-100 characters
- `script_command` MUST be resolved (not contain "{SCRIPT}" placeholder)
- `body` MUST NOT contain "/speckit." references (MUST use "/twitterkit.")
- `body` MUST NOT contain unresolved placeholders like "{SCRIPT}", "{ARGS}"

**YAML Frontmatter Structure**:
```yaml
---
agent: twitterkit.specify
description: Create or update the feature specification from a natural language feature description.
---
```

**Example** (twitterkit.specify.md):
```markdown
---
agent: twitterkit.specify
description: Create or update the feature specification from a natural language feature description.
...
9. Write to agent-specific directory with twitterkit prefix
...
- `release_name` (string, required): Display name (e.g., "twitter Kit Templates - 0.1.0")
...
```json
{
  ...
  "description": "Verify all commands use twitterkit.* namespace",
  "passed": true,
  "details": "All 9 command files have correct agent: twitterkit.* frontmatter"
}
```

**Validation Checks** (from spec FR-018 to FR-022):
1. **Frontmatter Namespace**: All command files use `agent: twitterkit.*` (not `speckit.*`)
...
5. **Constitution Version**: Includes twitter-Kit constitution v1.0.0
...
3. **Namespace Consistency**: All CommandFiles MUST use "twitterkit.*" namespace
...
To add a new command (e.g., "twitterkit.review"):
