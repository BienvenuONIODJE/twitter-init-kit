...
A twitter-Kit maintainer wants to create a new release with templates for all supported AI agents (Claude, Cursor, Windsurf, Gemini, Copilot, Qoder, etc.) using both bash and PowerShell script variants. They should be able to trigger the release process that automatically generates 36 template ZIP files and publishes them to GitHub releases.

**Why this priority**: This is the core capability that enables twitter-Kit to have its own releases and stop relying on Spec-Kit template fallback.
...
2. **Given** the workflow is running, **When** it processes each agent/script combination, **Then** it creates correctly structured ZIP files with twitter-Kit branding and `/twitterkit.*` commands
...
4. **Given** a release is published, **When** users run `twitter init my-project`, **Then** the CLI downloads templates from twitter-kit releases (not spec-kit fallback)
...
2. **Given** templates are built locally, **When** the developer inspects a ZIP file, **Then** it contains the correct directory structure with agent-specific commands and twitter-Kit branding
...
The continuous integration system should validate that all templates are correctly formatted, contain required files, and have proper twitter-Kit branding before allowing merge to main.
...
2. **Given** CI is validating templates, **When** it finds `/speckit.*` references that should be `/twitterkit.*`, **Then** the build fails with clear error messages
...
- **FR-004**: Each template MUST contain twitter-Kit specific files: constitution.md (twitter principles), twitter-focused spec/plan/tasks templates, and twitter-specific command files
- **FR-005**: All agent command files MUST use `/twitterkit.*` command prefix (not `/speckit.*`)
...
- **FR-020**: Validation MUST scan for unintended `/speckit.*` references that should be `/twitterkit.*`
- **FR-021**: Validation MUST verify agent command frontmatter uses correct `agent: twitterkit.{command}` format
- **FR-022**: Validation MUST confirm all templates reference twitter-Kit (not Spec-Kit) in branding and documentation
...
- **Template Base**: Source directory structure containing twitter-Kit specific files before agent/script customization
...
- **SC-004**: After release, `twitter init` command successfully downloads templates from twitter-kit repository (not spec-kit fallback)

#### Template Quality

- **SC-005**: 100% of generated templates pass validation checks: correct structure, required files present, no Spec-Kit references
- **SC-006**: All templates contain `/twitterkit.*` command references (zero `/speckit.*` references remain)
- **SC-007**: Each template includes twitter-Kit constitution v1.0.0 with 7 twitter-specific principles
- **SC-008**: Templates extract successfully and create functional twitter projects when used with `twitter init`
...
- Template customization per agent beyond command directory names (all agents use same twitter templates)
...
- **twitter-Kit Constitution**: `/memory/constitution.md` - twitter-specific principles (v1.0.0) to include in all templates
- **twitter-Kit Templates**: `/templates/` directory - Source templates for spec, plan, tasks that need to be included in each variant
- **twitter-Kit Commands**: `/templates/commands/` - Command templates that define `/twitterkit.*` slash commands
- **CLI Template Downloader**: `/src/twitter_cli/__init__.py` - `download_template_from_github()` function that expects `spec-kit-template-{agent}-{script}.zip` naming
- **Template Fallback Logic**: Currently implemented fallback to spec-kit templates when twitter-kit releases don't exist (to be removed after first release)
