...
This research document resolves technical unknowns and documents technology decisions for implementing automated template release generation for twitter-Kit. The system will generate 36 template variants (18 agents × 2 script types) and publish them to GitHub releases, enabling twitter-Kit to have its own releases instead of relying on Spec-Kit fallback.
...
- Maintains backward compatibility with existing `twitter init` command

**Alternatives Considered**:
- **tar.gz format**: More common in Unix environments, but requires additional handling on Windows
- **twitter-kit-template-* naming**: Would require changing CLI code unnecessarily; keeping spec-kit naming maintains compatibility
...
**Rationale**:
- Existing twitter-Kit infrastructure uses Bash (setup-plan.sh, create-new-feature.sh, etc.)
...
- No custom agents needed for twitter-Kit's use case
...
**Decision**: Use `/twitterkit.*` prefix for all slash commands (not `/speckit.*`)

**Rationale**:
- Constitution Principle VI (Kit Namespace Isolation) requires unique namespaces
- Enables users to install both spec-kit and twitter-kit simultaneously
...
- **Use /twitter.* prefix**: Too short, potential conflicts with other tools
- **Use /specify.* prefix**: Conflicts with spec-kit's namespace

**Implementation Impact**:
- Validate all generated templates use `agent: twitterkit.{command}` in frontmatter
...
- Modify `create-release-packages.sh` to use twitter-Kit templates
...
├── .specify/
│   ├── memory/
│   │   └── constitution.md (twitter-Kit v1.0.0)
│   ├── scripts/
...
- **Different directory name (.twitterkit/)**: Would require CLI changes
...
**Validation Phases**:
1. **Frontmatter validation**: Ensure `agent: twitterkit.*` in all command files
2. **Content scanning**: Search for any `/speckit.*` references that should be `/twitterkit.*`
...
- Update text to reference "twitter CLI" instead of "Specify CLI"
...
### Unknown 1: How to prevent Spec-Kit references from leaking into twitter-Kit templates?

**Resolution**: Multi-layered prevention strategy

1. **Source Control**: Keep templates/ directory with twitter-Kit specific content only
...
4. **Frontmatter Check**: Ensure all commands use `agent: twitterkit.*`
...
- Run `twitter init my-test-project --agent=claude --script=sh`
- Verify CLI downloads twitter-Kit template (not Spec-Kit fallback)
- Test each `/twitterkit.*` command works in agent
...
**Impact**: High (breaks twitter-Kit identity)
**Mitigation**:
- Automated validation in CI
- Grep for `/speckit.` patterns
- Frontmatter checks for `agent: twitterkit.*`
...
- Check fallback logic triggers only when twitter-Kit release missing
...
**Rationale**:
- Simpler mental model (one twitter-Kit version)
...
- **twitter-Kit Constitution**: `/memory/constitution.md` (v1.0.0)
- **Existing Build Script**: `.github/workflows/scripts/create-release-packages.sh`
- **CLI Template Downloader**: `/src/twitter_cli/__init__.py`
