# twitter-Kit Maintainer Guide

This guide explains how to maintain and update twitter-Kit templates, add new commands, and manage releases.
...
ls -la /tmp/test-template/.twitter/
...
for zip in dist/templates/*.zip; do
  unzip -l "$zip" | grep "twitterkit.mycommand" && echo "✓ Found in $(basename $zip)"
done

# Extract and test a variant
unzip -d /tmp/test dist/templates/spec-kit-template-claude-sh-v0.1.1.zip
grep -A2 "## User Input" /tmp/test/.claude/commands/twitterkit.mycommand.md
```
...
git commit -m "feat: Add /twitterkit.mycommand slash command"
git push
```
...
unzip -l dist/templates/spec-kit-template-newagent-sh-v0.1.1.zip | grep twitterkit
unzip -l dist/templates/spec-kit-template-newagent-ps-v0.1.1.zip | grep twitterkit
```
...
```bash
git add -A
git commit -m "feat: Add NewAgent support to twitter-Kit templates"
git push
```
...
- **MAJOR (X.0.0)**: Breaking changes to template structure or constitution principles removed
  - Example: Change directory structure from `.twitter/` to `.spec/`
  - Requires manual version bump

- **MINOR (0.X.0)**: New commands, new agents, new constitution principles
  - Example: Add `/twitterkit.review` command or support for 19th agent
...
gh release create v1.0.0 dist/templates/*.zip \
  --title "twitter Kit v1.0.0 - Major Release" \
...
# Fix with sed
sed -i 's/speckit/twitterkit/g' templates/commands/*.md
...
- [ ] Constitution is twitter-Kit v1.0.0
...
git clone https://github.com/agentii-ai/twitter-kit.git
cd twitter-kit
...
# Commit and push
git add -A
git commit -m "feat: Add new twitter-Kit feature"
git push origin add-new-command
```

## Getting Help

- **Questions**: [GitHub Discussions](https://github.com/agentii-ai/twitter-kit/discussions)
- **Issues**: [GitHub Issues](https://github.com/agentii-ai/twitter-kit/issues)
- **Code**: [View on GitHub](https://github.com/agentii-ai/twitter-kit)
