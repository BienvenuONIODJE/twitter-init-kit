...
- ✅ **Script files** (`.twitter/scripts/`)
- ✅ **Template files** (`.twitter/templates/`)
- ✅ **Shared memory files** (`.twitter/memory/`) - **⚠️ See warnings below**
...
**Known issue:** `specify init --here --force` currently overwrites `.twitter/memory/constitution.md` with the default template, erasing any customizations you made.

**Workaround:**

```bash
# 1. Back up your constitution before upgrading
cp .twitter/memory/constitution.md .twitter/memory/constitution-backup.md

# 2. Run the upgrade
specify init --here --force --ai copilot

# 3. Restore your customized constitution
mv .twitter/memory/constitution-backup.md .twitter/memory/constitution.md
```
...
git restore .twitter/memory/constitution.md
```

### 2. Custom template modifications

If you customized any templates in `.twitter/templates/`, the upgrade will overwrite them. Back them up first:

```bash
# Back up custom templates
cp -r .twitter/templates .twitter/templates-backup
...
rm twitterkit.specify-old.md
rm twitterkit.plan-v1.md
```
...
cp .twitter/memory/constitution.md /tmp/constitution-backup.md
cp -r .twitter/templates /tmp/templates-backup
...
mv /tmp/constitution-backup.md .twitter/memory/constitution.md
...
rm twitterkit.old-command-name.md
...
cp /tmp/constitution-backup.md .twitter/memory/constitution.md
```
...
3. **Some files may be overwritten** - If you already have Spec Kit files (`.claude/`, `.twitter/`, etc.), they'll be replaced with the new versions

**What gets overwritten:**

Only Spec Kit infrastructure files:
- Agent command files (`.claude/commands/`, `.github/prompts/`, etc.)
- Scripts in `.twitter/scripts/`
- Templates in `.twitter/templates/`
- Memory files in `.twitter/memory/` (including constitution)
...
**Prevention tip:** Before upgrading, commit or back up your `.twitter/memory/constitution.md` if you customized it.
...
Once you've run `specify init`, the slash commands (like `/twitterkit.twitter`, `/twitterkit.plan`, etc.) are **permanently installed** in your project's agent folder (`.claude/`, `.github/prompts/`, etc.). Your AI assistant reads these command files directly—no need to run `specify` again.
...
- **Test new slash commands:** Run `/twitterkit.constitution` or another command to verify everything works
